...
query, orderBy, onSnapshot, updateDoc, serverTimestamp, DocumentData
...
type Page = “Home” | “About” | “Chat” | “Contact” | “Feedback” | “Auth” | “Admin”;
...
background: “#0a0a0a”, border: “1px solid #222”, borderRadius: 20, padding: “40px 40px”
};

const label: React.CSSProperties = {
fontSize: 13, color: “#888”, marginBottom: 6, display: “block”
};

function primaryBtn(disabled = false): React.CSSProperties {
return {
background: disabled ? “#333” : “#fff”, color: disabled ? “#666” : “#000”,
border: “none”, borderRadius: 12, padding: “12px 24px”,
fontWeight: 600, fontSize: 14, cursor: disabled ? “not-allowed” : “pointer”, width: “100%”
};
}
...
padding: “12px 24px”, fontWeight: 600, fontSize: 14, cursor: “pointer”, width: “100%”
};
}

// ─── Navbar ───────────────────────────────────────────────────────────────────
function Navbar({ page, setPage, user, isAdmin }: {
page: Page; setPage: (p: Page) => void; user: User | null; isAdmin: boolean;
}) {
const links: Page[] = [“Home”, “About”, “Chat”, “Contact”, “Feedback”];
return (
<nav style={{
display: “flex”, alignItems: “center”, justifyContent: “space-between”,
padding: “0 32px”, height: 56, borderBottom: “1px solid #1a1a1a”,
background: “#000”, position: “sticky”, top: 0, zIndex: 100
}}>
<div style={{ display: “flex”, alignItems: “center”, gap: 8, fontWeight: 600, fontSize: 15, cursor: “pointer”, color: “#fff” }}
...
background: “none”, border: “none”, color: page === l ? “#fff” : “#666”,
fontWeight: page === l ? 600 : 400, fontSize: 14, cursor: “pointer”, padding: 0
}}>{l}</button>
))}
{isAdmin && (
...
background: “none”, border: “none”, color: “#fff”, fontWeight: 600, fontSize: 14, cursor: “pointer”
}}>Sign out</button>
) : (
...
<h2 style={{ fontSize: 28, fontWeight: 700, marginBottom: 8, textAlign: “center”, color: “#fff” }}>
...
{error && (
<div style={{ background: “#1a0000”, border: “1px solid #500”, borderRadius: 10, padding: “10px 14px”, color: “#f88”, fontSize: 13, marginBottom: 16 }}>{error}</div>
)}
...
<span style={{ color: “#fff”, cursor: “pointer”, fontWeight: 600 }}
...
padding: “14px 28px”, fontWeight: 600, fontSize: 15, cursor: “pointer”
...
Axid AI is your all-in-one futuristic AI assistant — designed to feel calm, professional, and fast.
</p>
...
<button onClick={() => setPage(“Chat”)} style={{ …primaryBtn(), marginTop: 40, width: “auto”, padding: “14px 28px” }}>
Start chatting
</button>
</div>
);
...
// ─── Chat Page ────────────────────────────────────────────────────────────────
function ChatPage({ user, setPage }: { user: User | null; setPage: (p: Page) => void }) {
...
const q = query(collection(db, “users”, user.uid, “sessions”, activeSession, “messages”), orderBy(“createdAt”));
const unsub = onSnapshot(q, snap => {
...
const send = async () => {
if (!input.trim() || loading || !activeSession || !user) return;
...
try {
  ...
  const reply: string = data.candidates?.[0]?.content?.parts?.[0]?.text ?? "Sorry, I couldn't respond.";
...
<button onClick={newSession} style={{ …secondaryBtn(), borderRadius: 10, padding: “10px 16px”, textAlign: “left” }}>
+ New chat
</button>
</div>
...
padding: “10px 16px”, color: activeSession === s.id ? “#fff” : “#777”,
fontSize: 13, cursor: “pointer”, background: activeSession === s.id ? “#1a1a1a” : “transparent”,
borderRadius: 8, margin: “2px 8px”, display: “flex”, alignItems: “center”, gap: 8
}}>
...
<button onClick={() => signOut(auth)} style={{ background: “none”, border: “none”, color: “#666”, fontSize: 13, cursor: “pointer”, padding: 0 }}>
Sign out
</button>
</div>
</div>
  ...
  <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20, maxWidth: 720, margin: "0 auto" }}>
          {messages.map(m => (
            <div key={m.id} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
              {m.role === "assistant" && (
                <div style={{ width: 28, height: 28, borderRadius: "50%", background: "#1a1a1a", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, marginRight: 10, flexShrink: 0, marginTop: 4, color: "#fff" }}>A</div>
              )}
              <div style={{
          ...
          {loading && (
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 28, height: 28, borderRadius: "50%", background: "#1a1a1a", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: "#fff" }}>A</div>
    ...
    <div style={{ padding: "16px 60px 24px", borderTop: "1px solid #111" }}>
      <div style={{ display: "flex", alignItems: "center", background: "#111", border: "1px solid #2a2a2a", borderRadius: 16, padding: "10px 16px", gap: 12, maxWidth: 720, margin: "0 auto" }}>
        ...
        <button onClick={send} disabled={loading || !input.trim() || !activeSession} style={{
          background: input.trim() && activeSession ? "#fff" : "#222",
          border: "none", borderRadius: 10, width: 34, height: 34,
          cursor: "pointer", fontSize: 14,
          color: input.trim() && activeSession ? "#000" : "#555",
...
// ─── Feedback Page ────────────────────────────────────────────────────────────
function FeedbackPage({ user }: { user: User | null }) {
...
<h1 style={{ fontSize: 52, fontWeight: 700, marginBottom: 12, color: “#fff” }}>Feedback</h1>
...
{submitted ? (
<div style={{ textAlign: “center”, fontSize: 20, padding: “20px 0”, color: “#fff” }}>Thank you for your feedback! 🎉</div>
) : (
<>
...
<button onClick={submit} style={{ …primaryBtn(), marginTop: 16 }}>Submit Feedback</button>
</>
)}
</div>
</div>
...
<h1 style={{ fontSize: 52, fontWeight: 700, marginBottom: 16, color: “#fff” }}>Contact Axid AI</h1>
...
<div style={{ fontWeight: 600, fontSize: 17, marginBottom: 6, color: “#fff” }}>{c.title}</div>
<div style={{ color: “#888”, fontSize: 13, marginBottom: 20 }}>{c.sub}</div>
<a href={c.href} target="_blank" rel="noreferrer">
<button style={{ …secondaryBtn(), borderRadius: 10, padding: “11px 0”, fontSize: 13 }}>{c.btn}</button>
</a>
</div>
))}
</div>
...
const [userChats, setUserChats] = useState<Session[]>([]);
...
const unsub2 = onSnapshot(query(collection(db, “feedback”), orderBy(“createdAt”, “desc”)), snap => {
...
setUserChats(snap.docs.map(d => ({ id: d.id, …d.data() } as Session)));
};

const loadSessionMessages = async (uid: string, sessionId: string) => {
const q = query(collection(db, “users”, uid, “sessions”, sessionId, “messages”), orderBy(“createdAt”));
const snap = await getDocs(q);
setSessionMessages(snap.docs.map(d => ({ id: d.id, …d.data() } as ChatMessage)));
};
...
<div>
<h1 style={{ fontSize: 36, fontWeight: 700, margin: 0, color: “#fff” }}>Admin Panel</h1>
<p style={{ color: “#555”, fontSize: 14, marginTop: 4 }}>Logged in as {user?.email}</p>
</div>
<div style={{ display: “flex”, gap: 10 }}>
<button style={tabStyle(“users”)} onClick={() => { setTab(“users”); setSelectedUser(null); setSelectedSession(null); }}>
Users ({users.length})
</button>
...
```
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      {users.map(u => (
        <div key={u.id} style={{ background: "#0a0a0a", border: "1px solid #1e1e1e", borderRadius: 14, padding: "18px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ fontWeight: 600, fontSize: 15, color: "#fff" }}>{u.name || "No name"}</div>
            <div style={{ color: "#666", fontSize: 13, marginTop: 2 }}>{u.email}</div>
            {u.blocked && <div style={{ color: "#f44", fontSize: 12, marginTop: 4 }}>● Blocked</div>}
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={() => { setSelectedUser(u); loadUserChats(u.id); }} style={{
              background: "#1a1a1a", border: "none", borderRadius: 8, color: "#fff", padding: "8px 14px", fontSize: 13, cursor: "pointer"
            }}>View Chats</button>
            <button onClick={() => toggleBlock(u)} style={{
              background: u.blocked ? "#1a0000" : "#0a1a0a",
              border: `1px solid ${u.blocked ? "#500" : "#050"}`,
              borderRadius: 8, color: u.blocked ? "#f66" : "#6f6", padding: "8px 14px", fontSize: 13, cursor: "pointer"
  ...
  {tab === "users" && selectedUser && !selectedSession && (
    <div>
      <button onClick={() => { setSelectedUser(null); setUserChats([]); }} style={{ background: "none", border: "none", color: "#888", fontSize: 14, cursor: "pointer", marginBottom: 20 }}>
        ← Back to users
      </button>
      <h2 style={{ fontSize: 22, fontWeight: 600, marginBottom: 20, color: "#fff" }}>Chats for {selectedUser.name || selectedUser.email}</h2>
      {userChats.length === 0 && <div style={{ color: "#555", textAlign: "center", padding: 40 }}>No chats yet.</div>}
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {userChats.map(s => (
          <div key={s.id} onClick={() => { setSelectedSession(s); loadSessionMessages(selectedUser.id, s.id); }} style={{
            ...
            <span style={{ fontSize: 15, color: "#fff" }}>{s.title || "Untitled chat"}</span>
  ...
  {tab === "users" && selectedUser && selectedSession && (
    <div>
      <button onClick={() => { setSelectedSession(null); setSessionMessages([]); }} style={{ background: "none", border: "none", color: "#888", fontSize: 14, cursor: "pointer", marginBottom: 20 }}>
        ← Back to chats
      </button>
      <h2 style={{ fontSize: 20, fontWeight: 600, marginBottom: 24, color: "#fff" }}>{selectedSession.title}</h2>
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        {sessionMessages.map(m => (
          <div key={m.id} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
            <div style={{
              background: m.role === "user" ? "#1a1a1a" : "#0d0d0d",
              border: "1px solid #222", borderRadius: 14, padding: "12px 18px",
              maxWidth: "80%", color: "#d0d0d0", fontSize: 14, lineHeight: 1.6, whiteSpace: "pre-wrap"
            }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {feedback.map(f => (
        <div key={f.id} style={{ background: "#0a0a0a", border: "1px solid #1e1e1e", borderRadius: 14, padding: "20px 24px" }}>
          ...
          <p style={{ color: "#ccc", fontSize: 15, lineHeight: 1.6, margin: 0 }}>
            {f.text || <span style={{ color: "#555" }}>No written feedback.</span>}
          </p>
        </div>
      ))}
    </div>
...
{page === “Chat” && <ChatPage user={user} setPage={setPage} />}
